 ...
 *   2. Create project "Discover Vietnam"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Vietnam Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'vietnam'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Vietnam ${year}`,
      description: `${title}\n\nFull guide: https://discovervietnam.info/${slug}/`,
      tags: [`Vietnam ${slug}`, 'Vietnam travel', `Vietnam ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Vietnam ✈️ #shorts`,
      description: `${title} — Full guide: https://discovervietnam.info/destinations/${slug}/`,
      tags: [`${title} Vietnam`, 'Vietnam travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Vietnam — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Vietnam for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discovervietnam.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discovervietnam.info/companion/

#Vietnam #${title.replace(/\s+/g, '')} #VietnamTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  ...
  if (dest?.region) tags.push(`${dest.region} Vietnam`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER VIETNAM                ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
